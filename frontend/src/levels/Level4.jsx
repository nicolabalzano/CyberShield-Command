...
- HttpOnly Cookies: ${protectionsEnabled.httpOnlyCookies ? '✓' : '✗'}`;
    ...
    const getHintText = () => {
        ...
        switch(currentStep) {
            case 0:
                return '🔍 STEP 1: Analizza i commenti nel forum. Vedi tag <script> o attributi strani? Usa "analyze-comments" per i dettagli.';
            case 1:
                return '🛡️ STEP 2: Hai identificato XSS Stored! Usa "enable-sanitization" per bloccare i tag pericolosi.';
            case 2:
                return '🔄 STEP 3: Attiva anche "enable-csp" per protezione extra, poi riavvia con "restart-app".';
            default:
        ...
        <div>
            ...
            <div className="fixed top-18 left-1/2 -translate-x-1/2 z-[15]">
                <div className="text-cyan-400 text-lg font-mono flex items-center gap-4">
                    <div className="flex items-center gap-2">
                        <span className="font-bold">XSS STATUS:</span>
                        <span className={`text-xl font-bold ${
                            !attackActive ? 'text-green-500' :
                            scriptExecuted ? 'text-red-500 animate-pulse' :
                            'text-yellow-500'
                        }`}>
                            {!attackActive ? 'SECURE' : scriptExecuted ? 'ACTIVE' : 'PARTIAL'}
                        </span>
                    </div>
                    <div className="text-sm">
                        {attackActive && scriptExecuted ? (
                            <span className="text-red-500 font-bold animate-pulse">🚨 SCRIPTS EXECUTING</span>
                        ) : attackActive ? (
                            <span className="text-yellow-500 font-bold">⚠️ VULNERABLE</span>
                        ) : (
                            <span className="text-green-500 font-bold">🔒 PROTECTED</span>
                        )}
                    </div>
                </div>
            </div>
